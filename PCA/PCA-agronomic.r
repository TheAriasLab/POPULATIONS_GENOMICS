...
ploidy(gl.coco) <- 2
agro <- read.table("./../PCA-agronomic/agronomic_2.tsv", sep = "\t", header = T)
...
d_agronomic <- merge(d, agro, by = "ind")
# make plot
# labels
p <- ggplot(d_agronomic, aes(x = PC1, y = PC2, shape = factor(Origin), color = Water)) 
p <- p + geom_point(size=3.5, alpha = 1)
p <- p + labs(title = "", col="Water", shape="Origin")
...
p
ggsave(p, filename = "./PCA_nomixed_labels_Water-Zissou1.png", units = "cm", width = 15*1.3, height = 15, dpi = 320)
